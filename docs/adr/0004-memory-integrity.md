...
After diagnosis, the orchestrator captures only independent artefacts outside the injected path set, restores the pre-intervention checkpoint, rewrites the retained investigator artefacts, and validates the sanitized Wake Report. The controlled mutation is therefore available as an experience but can never be promoted as returned code.
