...
| [Concept fidelity](./CONCEPT_FIDELITY.md) | Which original Reality/Dream guarantees are implemented and preserved? |
| [ADRs](./adr/) | Which architectural decisions are deliberate? |

The password-reset Demo Mission proves nested integrity-sealed memory in deterministic recording mode and can also run live. Mission Control defaults to a pinned VAmPI defensive-maintenance target, accepts arbitrary trusted local Git repositories in real mode, and supports competing sibling Dreams in one branching Reality graph.
