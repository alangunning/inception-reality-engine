...
Reality Engine reuses the judge's CLI authentication while isolating personal Codex plugins and MCPs by default. Codex retains writable, network-enabled execution inside each Reality worktree; explicit inspection of the judge's home directory, root filesystem, Git reflogs, refs, or sibling worktrees is rejected by the validated command policy. This makes the evaluation reproducible even when the judge has unrelated integrations configured; `INCEPTION_CODEX_INHERIT_USER_CONFIG=true` is an explicit opt-in for missions that need them.

**Mission Control** is available at `/missions`. Its default is a pinned VAmPI repository-maintenance Mission for one documented ownership regression. Preparing that allowlisted target is explicit and makes no Codex call; it creates a verified fixture commit containing a dependency-free baseline proof plus a parent-authorized pinned framework manifest on top of the exact upstream revision. Deep integration Dreams receive an isolated `.venv` using whichever Python 3 executable is already available; no global package or interpreter is changed. Creating a Mission creates only Git state. The first Codex execution begins after the explicit local source-review action. The prompt prohibits running a service or contacting targets, accounts, credentials, or network systems outside the approved package download. The Adversarial Subject is enabled by default at Dream depth two, remains bounded to configured local paths, fault class, files, lines, tokens, time, and a rollback checkpoint, and can be disabled before creating the Mission.
